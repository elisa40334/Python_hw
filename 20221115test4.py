...
class treeNode:
    ...
    def disp(self, ind=1):
        ...
        for child in self.children.values():
            ...
            # last = ind+1
    def last(self,lista,ind = 1):
        lista += [self.count]
        for child in self.children.values():
            child.last(lista,ind+1)
...
# part 3 : 创建FP树
def createTree(dataSet, minSup=813):
    '''
...
def findPrefixPath(basePat, treeNode):
    ...
    while treeNode is not None: # 对 treeNode的link进行循环
        ...
        if len(prefixPath) > 0: # 排除自身这个元素，判断是否存在父元素（所以要>1, 说明存在父元素）
...
# part 5 : 递归查找频繁项集
def mineTree(inTree, headerTable, minSup, preFix, freqItemList,hihi,count):
    '''
    递归查找频繁项集
   (创建条件FP树)
    ...
    for basePat in bigL:
        ...
        # print("headerTable[basePat][1]",headerTable[basePat][1])
        ...
        if myHead is not None:
            # myCondTree.disp(1)
            lista = []
            myCondTree.last(lista,1)
            # print(lista[-1])
            # for expression in myCondTree.last(lista,1):
            #     print(expression)
            # print(next())
            # print(last,'\n\n\n')
            # 递归 myHead 找出频繁项集
            ...
            mineTree(myCondTree, myHead, minSup, newFreqSet, freqItemList,hihi,count)
        else:
            if frozenset() in condPathBases and (len(newFreqSet) < 5):
                hihi += [condPathBases[frozenset()]]
            else:
                hihi += [1]
            # else:
            #     print
        # print('\n\n\n')
def findans(freqItemList,hihi,count,confi = 0.8):
    for i in range(0,len(freqItemList)):
        # if len(freqItemList[i]) == 1:
        #     count[1] += 1
        #     count[0] += 1
        for j in range(i+1,len(freqItemList)):
            # print("aa")
            if len(freqItemList[i])+len(freqItemList[j]) > 5:
                pass
            elif len(freqItemList[i])+len(freqItemList[j]) == 2:
                # print(count[2])
                if hihi[i]/hihi[j] >= confi:
                    count[2] += 1
                    count[0] += 1
            elif len(freqItemList[i])+len(freqItemList[j]) == 3:
                if hihi[i]/hihi[j] >= confi:
                    count[3] += 1
                    count[0] += 1
            elif len(freqItemList[i])+len(freqItemList[j]) == 4:
                if hihi[i]/hihi[j] >= confi:
                    count[4] += 1
                    count[0] += 1
            elif len(freqItemList[i])+len(freqItemList[j]) == 5:
                if hihi[i]/hihi[j] >= confi:
                    count[5] += 1
                    count[0] += 1
...
# myFPtree.disp()
# print('A --->', findPrefixPath('A', myHeaderTab['A'][1]))
...
hihi = []
count = [0,0,0,0,0,0]
mineTree(myFPtree, myHeaderTab, minSup, set([]), freqItemList,hihi,count)
# print("myHeaderTab",myHeaderTab)
# print("freqItemList: \n", freqItemList)
# print(hihi)

# findans(freqItemList,hihi,count,confi = 0.8)
for i in freqItemList:
    if len(i) == 1:
        ...
        count[5] += 1
print(count[1],":,",count[2],":",count[3],":",count[4],":",count[5],":",count[0],":",len(freqItemList))
print("end")
